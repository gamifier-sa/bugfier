...
let KTDatatable = function () {
    ...
    let initDatatable = function () {
        let waitingLabStatus = $('#waitingLabStatus').val();
        ...
        datatable = $("#kt_datatable").DataTable({
            ...
            ajax: {
                data: function () {
                    ...
                    let searchWith = $('#searchWithColumn').val();
                    if(FROMDATE || TODATE){
                        datatable.DataTable().ajax.url(`/dashboard/waiting-labs/${waitingLabStatus}?page=${info.page + 1}&search_with=${searchWith}&per_page=${info.length}&patient=${PAITiENT_ID}&from_date=${FROMDATE}&to_date=${TODATE}`);
                    }else{
                        datatable.DataTable().ajax.url(`/dashboard/waiting-labs/${waitingLabStatus}?page=${info.page + 1}&search_with=${searchWith}&per_page=${info.length}&patient=${PAITiENT_ID}`);
                    }
                }
            },
            columns: [
            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        return `<a href="/dashboard/waiting-labs/${waitingLabStatus}?patient=${row.patient.id}" style="color:#5b0efa;"> ${row.patient.name_ar} </a>`;
    ...
    $('#searchWithColumn').change( function () {
        datatable.draw();
    });
