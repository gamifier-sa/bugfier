...
@section('content')
    ...
    <div class="d-flex flex-column align-items-end gap-5 flex-wrap mb-5 container-fluid">
        ...
        <div class="d-flex justify-content-end" data-kt-docs-table-toolbar="base">
            <!--begin::Filter-->
            <button type="button" class="btn my-gold me-3" data-kt-menu-trigger="click"
            ...
            <div class="menu menu-sub menu-sub-dropdown w-300px w-md-325px" data-kt-menu="true">
                ...
                <div class="px-7 py-5" data-kt-docs-table-filter="form">
                    ...
                    <div class="mb-10">
                        ...
                        <select class="form-select form-select-solid fw-bold" name="status_id" data-kt-select2="true"
                            data-placeholder="{{ __('Please Choose') }}" data-allow-clear="true"
                            data-kt-docs-table-filter="search" data-hide-search="true"
                            data-dir="{{ isArabic() ? 'rtl' : 'ltr' }}">
                            <option></option>
                            @foreach ($statuses as $status)
                                <option @selected(old('status') == $status->id) value="{{ $status->id }}">{{ __($status->title) }}
                                </option>
                            @endforeach
                        </select>
                    </div><!--end::Input group-->
                    <!--begin::Input group-->
                    <div class="mb-10">
                        <label class="form-label fs-6 fw-semibold">{{ __('Created By') }}</label>
                        <select class="form-select form-select-solid fw-bold" name="created_by" data-kt-select2="true"
                            data-placeholder="{{ __('Please Choose') }}" data-allow-clear="true"
                            data-kt-docs-table-filter="search" data-hide-search="true"
                            data-dir="{{ isArabic() ? 'rtl' : 'ltr' }}">
                            <option></option>
                            @foreach ($admins as $admin)
                                <option @selected(old('created_by') == $admin->id) value="{{ $admin->id }}">{{ __($admin->name_en) }}
                                </option>
                            @endforeach
                        </select>
                    </div><!--end::Input group-->
            ...
            @can('create_bugs')
                ...
                <a href="{{ route('dashboard.bugs.create') }}" class="btn my-gold" data-bs-toggle="tooltip" title="">
...
">
                        ...
                        <th>
                            <div class='d-flex my-td-inner align-items-center justify-center for-date'
                                style="background-color: #8E6FAF;
">
                                <h3 class="fs-6"> {{ __('Created By') }}</h3>
                            </div>
                        </th>
                        <th>
                            <div class='d-flex my-td-inner align-items-center justify-center for-date'
